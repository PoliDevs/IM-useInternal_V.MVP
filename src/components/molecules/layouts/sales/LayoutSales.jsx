...
import { getDateCurrent,getLastMonday,roundToTwoDecimalPlaces } from "../../../../utils/functions";
...
export default function LayoutSales() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `order/vtas/${comerceId}?dateFrom=${monday}&dateTo=2023-10-03`
        );
  ...
  const today_orders_money=week&&week.find(cur=>cur.date==="2023-10-03");// pedidos y ventas de hoy
  ...
  return (
    <div className={s.containerd}>
      <div className={s.content_1}>
        ...
        <span>
        Total semanal:$
          <b style={{ fontWeight: "700" }}>{totalWeek&&roundToTwoDecimalPlaces(totalWeek)} </b>
        </span>
